...
class DiagnoseInfo {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new DiagnoseInfo(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.info[i] = KeyValue.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new DiagnoseInfo(null);
    ...
    if (msg.info !== undefined) {
      ...
      for (let i = 0; i < resolved.info.length; ++i) {
        resolved.info[i] = KeyValue.Resolve(msg.info[i]);
      }
    }
    else {
      resolved.info = []
